...
This module provides profiling utilities for measuring GPU kernel performance,
including latency, throughput, and memory bandwidth utilization.
...
Usage
-----
>>> from flashtile.utils import KernelProfiler
...
Notes
-----
For detailed kernel analysis, use NVIDIA Nsight Systems or Nsight Compute:
...
class CUDATimer:
    ...
    Uses CUDA events for accurate GPU timing, including handling
...
class KernelProfiler:
    """
    GPU kernel profiler with comprehensive metrics collection.

    This profiler measures kernel performance including latency,
    throughput, and memory bandwidth utilization.

    Parameters
    ----------
    warmup : int, optional
        Number of warmup iterations. Default is 10.

    iterations : int, optional
        Number of timed iterations. Default is 100.

    peak_tflops_fp16 : float, optional
        Peak FP16 TFLOPS for the GPU. Default is 312 (A100).

    peak_bandwidth_tb : float, optional
        Peak memory bandwidth in TB/s. Default is 2.0 (A100).

    Examples
    --------
    >>> profiler = KernelProfiler(warmup=5, iterations=50)
    >>> result = profiler.profile_function(
    ...     fn=lambda: model(x),
    ...     name="flash_attention",
    ...     flops=compute_flops,
    ...     memory_bytes=compute_memory,
    ... )
    >>> print(result)
    """
    ...
    def profile_function(
        ...
        **kwargs,
        ...
        name : str
            Name for this profiling run.

        flops : int, optional
            Number of floating-point operations for throughput calculation.

        memory_bytes : int, optional
            Number of bytes accessed for bandwidth calculation.

        **kwargs
            Additional metadata to store with results.
    ...
    def profile(self, name: str):
        ...
        Note: This only measures latency. Use profile_function for
        ...
        ------
        dict
            Dictionary that will contain 'latency_ms' after the block.

        Example
        -------
        >>> with profiler.profile("my_kernel") as result:
        ...     output = model(x)
    ...
    def compare_implementations(
        ...
        flops : int, optional
            FLOPs for all implementations (assumed same).

        memory_bytes : int, optional
            Memory bytes for all implementations.

        Returns
        -------
        dict
            Mapping of name to ProfileResult.
        """
        results = {}
...
def profile_attention_implementations(
    ...
    """
    Profile all available attention implementations.

    This is a convenience function for comparing FlashTile's
    ...
    -------
    dict
        Mapping of implementation name to ProfileResult.
    """
