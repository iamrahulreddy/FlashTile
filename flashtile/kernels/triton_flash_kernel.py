"""
Triton Flash Attention Kernel (Forward-Only)
============================================

This module provides a high-performance GPU kernel for Flash Attention
implemented using Triton. It achieves near-optimal memory throughput
by explicitly managing data movement between HBM (GPU memory) and
SRAM (on-chip cache).

.. important::
    **Forward-Only**: This Triton kernel implements only the forward pass.
    For training with backpropagation, use `FlashAttentionV1` or 
    `FlashAttentionV2` which have custom autograd functions.
...
---------------------------
The Triton kernel typically achieves:
- 2-4x speedup over Python implementation for long sequences
- Memory bandwidth efficiency approaching theoretical peak
- Automatic hardware adaptation for different GPU architectures

Algorithm
---------
The kernel implements the same Flash Attention algorithm as the Python
version but with explicit memory hierarchy management:

1. Divide Q, K, V into blocks that fit in SRAM
2. Load blocks explicitly to minimize HBM access
3. Use online softmax to avoid storing the full attention matrix
4. Fuse operations to maximize arithmetic intensity

References
----------
1. Dao et al. "FlashAttention: Fast and Memory-Efficient Exact Attention"
2. Triton language and compiler: https://triton-lang.org/
"""
...
if HAS_TRITON:
    ...
    def _flash_attention_forward_kernel(
        ...
        Each program computes one block of output for one batch-head combination.
        The algorithm iterates over K/V blocks, computing partial attention
        and maintaining running softmax statistics.
        """
        # Get program IDs
        ...
        # [MEMORY HIERARCHY] Load Q block from HBM to SRAM
        # This determines the "tile" size (BLOCK_M x head_dim).
        # Once loaded, this Q block stays in SRAM while we iterate over all K/V blocks.
        # This reuse is why Flash Attention reduces HBM access by O(M)!
...
def triton_flash_attention(
    ...
    """
    Functional interface for Triton Flash Attention kernel.

    This function provides a drop-in replacement for standard attention
    with better performance on long sequences.
    ...
    ValueError
        If input tensors are not on CUDA or have incorrect shapes.
...
class TritonFlashAttention(BaseAttention):
    """
    Flash Attention layer using custom Triton kernel (Forward-Only).

    This module provides a high-performance attention implementation
    using Triton GPU kernels with explicit memory hierarchy management.
    
    .. important::
        **Forward-Only**: This class uses the Triton kernel which only
        implements the forward pass. Calling `.backward()` will fail.
        Run this module in inference mode (`model.eval()` + `torch.no_grad()`).
        Use `FlashAttentionV1` or `FlashAttentionV2` for training.
    
    Inherits from BaseAttention for consistent API with other implementations.
    ...
    dropout : float, optional
        Dropout probability (applied after attention). Default is 0.0.

    bias : bool, optional
        Use bias in projection layers. Default is True.

    causal : bool, optional
        Apply causal masking. Default is False.

    block_size : int, optional
        Block size for tiling (not used in Triton, kept for API consistency).
        Default is 64.
    ...
    Examples
    --------
    >>> model = TritonFlashAttention(embed_dim=512, num_heads=8).cuda()
    >>> x = torch.randn(2, 1024, 512).cuda()
    >>> model = model.eval()
    >>> with torch.no_grad():
    ...     output, _ = model(x)  # Returns (output, None) like other implementations
    """
    ...
    def forward(
        ...
        attn_mask : torch.Tensor, optional
            Custom attention masks are not supported by the Triton kernel.
            Use causal=True for causal masking. Raises NotImplementedError
            if a custom mask is provided.
    ...
    def get_memory_usage(self, batch_size: int, seq_len: int) -> Dict[str, Any]:
        """
        Calculate theoretical memory usage for Triton Flash Attention.
