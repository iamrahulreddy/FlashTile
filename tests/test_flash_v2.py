"""
Tests for FlashAttentionV2.

Validates V2-specific features:
- Swapped loop order produces correct output
- Causal masking works correctly
- Causal optimization skips appropriate blocks
"""
...
class TestCausalMasking:
    ...
    def test_causal_output_is_causal(self, device, seed):
        ...
        with torch.inference_mode():
            ...
            # Modify a future token (position 32)
            x_modified = x.clone()
            x_modified[0, 32, :] = torch.randn(embed_dim, device=device)
            out2, _ = model(x_modified)
        
        # Positions 0-31 should be UNCHANGED (they can't see position 32)
        ...
        # Positions 32+ should be DIFFERENT (they see the modified token)
